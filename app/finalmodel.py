...
data_combined['text'] = data_combined['text'].astype(str).apply(preprocess_text)
data_combined['sentiment'] = data_combined['sentiment'].map({'positive': 2, 'neutral': 1, 'negative': 0})
...
X_tfidf = get_tfidf_features(data_balanced['text'])

# Разделение на обучающую и тестовую выборки
X_train, X_test, y_train, y_test = train_test_split(X_tfidf, data_balanced['sentiment'], test_size=0.2, random_state=42)

# Нейронная сеть с улучшениями
model = Sequential([
    Dense(256, input_dim=X_train.shape[1], activation='relu'),
    Dropout(0.4),  # Увеличение dropout для борьбы с переобучением
    BatchNormalization(),
...
optimizer = Adam(learning_rate=0.0003)  # Еще большее уменьшение learning rate для улучшения сходимости
...
model.fit(X_train, y_train, epochs=30, batch_size=32, validation_data=(X_test, y_test), verbose=1, callbacks=[early_stopping])
